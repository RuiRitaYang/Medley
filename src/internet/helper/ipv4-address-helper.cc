...
Ipv4AddressHelper::AssignIPv4 (const NetDeviceContainer &c, Ipv4Address &src, Ipv4Address &dest)
{
  NS_LOG_FUNCTION_NOARGS ();
  Ipv4InterfaceContainer retval;
  // assign src ipv4
  Ptr<NetDevice> deviceSrc = c.Get(0);
  Ptr<Node> nodeSrc = deviceSrc->GetNode();
  NS_ASSERT_MSG (nodeSrc, "Ipv4AddressHelper::Assign(): NetDevice is not not associated "
                     "with any node -> fail");

  Ptr<Ipv4> ipv4Src = nodeSrc->GetObject<Ipv4> ();
  NS_ASSERT_MSG (ipv4Src, "Ipv4AddressHelper::Assign(): NetDevice is associated"
                     " with a node without IPv4 stack installed -> fail "
                     "(maybe need to use InternetStackHelper?)");

  int32_t interfaceSrc = ipv4Src->GetInterfaceForDevice(deviceSrc);
  if (interfaceSrc == -1){
    interfaceSrc = ipv4Src->AddInterface (deviceSrc);
  }
  NS_ASSERT_MSG (interfaceSrc >= 0, "Ipv4AddressHelper::Assign(): "
                     "Interface index not found");
  Ipv4InterfaceAddress ipv4AddrSrc = Ipv4InterfaceAddress (src, m_mask);
  ipv4Src->AddAddress(interfaceSrc,ipv4AddrSrc);
  ipv4Src->SetMetric(interfaceSrc,1);
  ipv4Src->SetUp(interfaceSrc);
  retval.Add(ipv4Src,interfaceSrc);

  // assign dest ipv4
  Ptr<NetDevice> deviceDest = c.Get(1);
  Ptr<Node> nodeDest = deviceDest->GetNode();
  NS_ASSERT_MSG (nodeDest, "Ipv4AddressHelper::Assign(): NetDevice is not not associated "
                     "with any node -> fail");

  Ptr<Ipv4> ipv4Dest = nodeDest->GetObject<Ipv4> ();
  NS_ASSERT_MSG (ipv4Dest, "Ipv4AddressHelper::Assign(): NetDevice is associated"
                     " with a node without IPv4 stack installed -> fail "
                     "(maybe need to use InternetStackHelper?)");

  int32_t interfaceDest = ipv4Dest->GetInterfaceForDevice(deviceDest);
  if (interfaceDest == -1){
    interfaceDest = ipv4Dest->AddInterface (deviceDest);
  }
  NS_ASSERT_MSG (interfaceDest >= 0, "Ipv4AddressHelper::Assign(): "
                     "Interface index not found");
  Ipv4InterfaceAddress ipv4AddrDest = Ipv4InterfaceAddress (dest, m_mask);
  ipv4Dest->AddAddress(interfaceDest, ipv4AddrDest);
  ipv4Dest->SetMetric(interfaceDest, 1);
  ipv4Dest->SetUp(interfaceDest);
  retval.Add(ipv4Dest,interfaceDest);
  return retval;
}
